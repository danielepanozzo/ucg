	...
	const std::string& filename)
{
	...
	for (unsigned wi = 0; wi < w; ++wi) {
		for (unsigned hi = 0; hi < h; ++hi) {
			data[(hi * w * 4) + (wi * 4) + 0] = double_to_unsignedchar(R(wi,hi));
			data[(hi * w * 4) + (wi * 4) + 1] = double_to_unsignedchar(G(wi,hi));
			data[(hi * w * 4) + (wi * 4) + 2] = double_to_unsignedchar(B(wi,hi));
			data[(hi * w * 4) + (wi * 4) + 3] = double_to_unsignedchar(A(wi,hi));
		}
	}
